    ...
    <div className="mt-5 rounded-xl bg-slate-50 px-4 py-3 dark:bg-slate-800/50">
      ...
      <ul className="mt-2 flex flex-col gap-1.5">
        {shown.map((line, i) => (
          <li
            key={i}
            className="text-base leading-relaxed break-words text-slate-800 dark:text-slate-200"
          >
            {line.replace(/^[-•]\s*/, '')}
          </li>
        ))}
      ...
      {overflowing ? (
        <button
          ...
          className="mt-3 text-base font-medium text-indigo-600 underline-offset-2 hover:underline dark:text-indigo-400"
        >
